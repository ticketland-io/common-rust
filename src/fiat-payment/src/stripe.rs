...
impl Stripe { 
  pub async fn create_link(&self, uid: String) -> Result<String> {
    let neo4j = Arc::clone(&self.neo4j);
    let ticketland_dapp = self.ticketland_dapp.clone();
    let uid_copy = uid.clone();

    let stripe_account = self.create_stripe_account().await?;
    let stripe_uid = stripe_account.id.clone();
    let account_link = self.create_stripe_account_link(
      stripe_uid.clone(),
      uid_copy.clone(),
    ).await?;
  ...
  pub async fn refresh_link(&self, uid: String) -> Result<String> {
    let neo4j = Arc::clone(&self.neo4j);
  ...
  pub async fn create_stripe_account(&self) -> Result<Account>  {
  ...
  pub async fn create_stripe_account_link(&self, stripe_uid: AccountId, uid: String,) -> Result<AccountLink> {
    let client = Client::new(self.stripe_key.clone());
    ...
    AccountLink::create(
      ...
      CreateAccountLink {
          ...
          expand: &[],
          refresh_url: Some(format!("{}/stripe/refresh-url?uid={}", &self.ticketland_dapp, &uid).as_str()),
          return_url: Some(format!("{}/stripe/return-url", &self.ticketland_dapp).as_str()),
      },
    )
    .await
    .map_err(Into::<_>::into)
  ...
  pub async fn create_checkout_session(
    ...
    let lock = self.redlock.lock(ticket_nft.as_bytes(), Duration::seconds(15).num_milliseconds() as usize).await?;
